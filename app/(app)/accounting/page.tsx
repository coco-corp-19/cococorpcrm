...
export default async function AccountingPage() {
  ...
  const [{ data: invoices }, { data: costs }, { data: cashflow }, { data: org }, { data: accounts }] = await Promise.all([
    ...
    supabase.from("fact_cashflow").select("balance, account_id, record_date").order("record_date", { ascending: false }),
  ...
  return (
    <section>
      <AccountingClient
        ...
        cashflow={(cashflow || []).map(r => ({ balance: Number(r.balance || 0), account_id: r.account_id, record_date: r.record_date || "" }))}
        bankTxns={bankTxns}
