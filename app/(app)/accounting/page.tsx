...
export default async function AccountingPage() {
  ...
  const [{ data: invoices }, { data: costs }, { data: income }, { data: cashflow }, { data: org }, { data: accounts }, { data: afsLines }, { data: capexProjects }, { data: capexTimes }] = await Promise.all([
    ...
    supabase.from("fact_costs").select("id, amount, transaction_date, cost_details, cost_category_id, cost_type, include_in_pnl, depreciation_months, residual_value, disposed_at, dim_cost_categories(name)").eq("org_id", orgId).is("deleted_at", null),
  ...
  const costRows = (costs || []).map(c => ({
    ...
    disposed_at: ((c as Record<string, unknown>).disposed_at as string | null) ?? null,
  }));
  ...
  return (
    <section>
      <AccountingClient
        ...
        costs={(costs || []).map(c => ({ id: c.id, amount: Number(c.amount || 0), transaction_date: c.transaction_date || "", cost_details: ((c as Record<string, unknown>).cost_details as string | null) ?? null, cost_category_id: c.cost_category_id, category_name: (c.dim_cost_categories as unknown as { name: string } | null)?.name ?? "Other", cost_type: ((c as Record<string, unknown>).cost_type as string) ?? "operational", include_in_pnl: (c as Record<string, unknown>).include_in_pnl !== false, depreciation_months: ((c as Record<string, unknown>).depreciation_months as number | null) ?? null, residual_value: Number((c as Record<string, unknown>).residual_value ?? 0), disposed_at: ((c as Record<string, unknown>).disposed_at as string | null) ?? null }))}
