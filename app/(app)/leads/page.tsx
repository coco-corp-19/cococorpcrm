...
export default async function LeadsPage() {
  ...
  const [{ data: leads }, { data: statuses }, { data: customers }, { data: products }, { data: org }] = await Promise.all([
    supabase
      .from("fact_leads")
      .select("id, name, phone, contact, lead_date, status_id, last_follow_up, opportunity_value, opportunity_weighted, weight, total_revenue, secured_revenue, contacted, responded, developed, paid, customer_id, product_id, created_at")
      .is("deleted_at", null)
  ...
  return (
    <section>
      <LeadsClient
        leads={(leads || []).map(l => ({ ...l, product_id: (l as Record<string, unknown>).product_id as number | null ?? null }))}
        statuses={statuses || []}
        customers={customers || []}
